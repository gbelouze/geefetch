...
# ---------------------------------------------------------------------------
# New helper: decide the export/output CRS in one place
# ---------------------------------------------------------------------------
def _resolve_output_crs(aoi_epsg: Optional[int], output_epsg: Optional[int]) -> Optional[CRS]:
    """
    Decide the export CRS used by download functions.

    Priority:
        1) If output_epsg is provided: use it (explicit user override).
        2) Else if AOI EPSG is provided and not 4326: use AOI EPSG (legacy behavior).
        3) Else: return None (let native/sensor CRS pass through).
    """
    if output_epsg is not None:
        return CRS.from_epsg(output_epsg)
    if aoi_epsg is not None and aoi_epsg != 4326:
        return CRS.from_epsg(aoi_epsg)
    return None


def load_country_filter_polygon(country: Any) -> shapely.Polygon | shapely.MultiPolygon | None:
    """Load the mainland shape of a country (or union of countries)."""
    match country:
        case str():
            country = [country]
        case None | []:
...
def save_config(
    ...
    # avoid persisting sensitive/project-specific info
    if "gee" in config and isinstance(config["gee"], dict) and "ee_project_ids" in config["gee"]:
...
def download_gedi(config_path: Path, vector: bool) -> None:
    ...
    # select default bands if not provided
    if vector:
        ...
        data.get.download_gedi_vector(
            ...
            crs=_resolve_output_crs(
                getattr(config.gedi.aoi.spatial, "epsg", None),
                getattr(config.gedi, "output_epsg", None),
            ),
...
def download_s1(config_path: Path) -> None:
    ...
    data.get.download_s1(
        ...
        crs=_resolve_output_crs(
            getattr(config.s1.aoi.spatial, "epsg", None),
            getattr(config.s1, "output_epsg", None),
        ),
...
def download_s2(config_path: Path) -> None:
    ...
    data.get.download_s2(
        ...
        crs=_resolve_output_crs(
            getattr(config.s2.aoi.spatial, "epsg", None),
            getattr(config.s2, "output_epsg", None),
        ),
...
def download_dynworld(config_path: Path) -> None:
    ...
    data.get.download_dynworld(
        ...
        crs=_resolve_output_crs(
            getattr(config.dynworld.aoi.spatial, "epsg", None),
            getattr(config.dynworld, "output_epsg", None),
        ),
...
def download_landsat8(config_path: Path) -> None:
    ...
    data.get.download_landsat8(
        ...
        crs=_resolve_output_crs(
            getattr(config.landsat8.aoi.spatial, "epsg", None),
            getattr(config.landsat8, "output_epsg", None),
        ),
...
def download_palsar2(config_path: Path) -> None:
    ...
    data.get.download_palsar2(
        ...
        crs=_resolve_output_crs(
            getattr(config.palsar2.aoi.spatial, "epsg", None),
            getattr(config.palsar2, "output_epsg", None),
        ),
...
def download_nasadem(config_path: Path) -> None:
    ...
    data.get.download_nasadem(
        ...
        bounds,
        crs=_resolve_output_crs(
            getattr(config.nasadem.aoi.spatial, "epsg", None),
            getattr(config.nasadem, "output_epsg", None),
        ),
...
def download_custom(config_path: Path, custom_name: str) -> None:
    ...
    data.get.download_custom(
        ...
        end_date,
        crs=_resolve_output_crs(
            getattr(custom_config.aoi.spatial, "epsg", None),
            getattr(custom_config, "output_epsg", None),
        ),
