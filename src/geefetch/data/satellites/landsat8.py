...
class Landsat8(SatelliteABC):
    ...
    def get_col(
        ...
        def getSunAngles(date, footprint):
            ...
            hourGMT = ee.Number(date.getRelative("second", "day")).divide(
                seconds_in_hour
            )
            ...
            # Hour as an angle
            ah = trueSolarTime.multiply(
                ee.Number(MAX_SATELLITE_ZENITH * 2).multiply(PI / 180)
            )
            b = ee.List(
        ...
        def azimuth(footprint):
            ...
            upperCenter = (
                line_from_coords(footprint, UPPER_LEFT, UPPER_RIGHT)
                .centroid()
                .coordinates()
            )
            lowerCenter = (
                line_from_coords(footprint, LOWER_LEFT, LOWER_RIGHT)
                .centroid()
                .coordinates()
            )
        ...
        def zenith(footprint):
            ...
            return viewZenith.multiply(PI / 180)

        def _applyL8(image, kvol, kvol0):
            # f_iso = 0
            # f_geo = 0
            # f_vol = 0
            blue = _correct_band(
        ...
        def _kvol(sunAz, sunZen, viewAz, viewZen):
            ...
            pa20 = (
                viewZen0.sin().multiply(sunZen.sin()).multiply(relative_azimuth.cos())
            )
